<div id="new-featured-slider">
	<div class="new-featured-slider--slide">

		<div class="new-featured-slider--slide--row">
			
			<div>
				<span class="genre"><a href="#">INTERVIEW</a> / <a href="">JAZZ - BOSSA NOVA</a></span>
				<h2 class="featured-title">FLAVIEN BERGER</h2>
				<h3 class="featured-subtitle">MADDY LA NUIT</h3>
				<p class="excerpt">Ut wisi enim ad minim veniam, quis nostrud exerci tation ullamcorper suscipit loborti.</p>
				<p class="author">par: <a href="">Alain Brunet</a></p>
			</div>
			<div>
				<img src="/dev/img/image-featured-home-slider-dev.png" />
			</div>
		</div>
		
	</div>
</div>